...
export async function runBenchmark(
	...
	const assertCondition = (name: string, condition: boolean, severity: string = "high") => {
		totalChecks++;
		if (condition) {
	...
	try {
		...
		assertCondition(
			"Project contains shared/constitution.md",
			fs.existsSync(path.join(sharedDir, "constitution.md")),
		);
		assertCondition(
			"Project contains shared/state.md",
			fs.existsSync(path.join(sharedDir, "state.md")),
		);
		...
		if (fs.existsSync(specsDir)) {
			...
			for (const feature of features) {
				...
				assertCondition(
					`Feature [${feature}] has spec.md`,
					fs.existsSync(path.join(fDir, "spec.md")),
				);

				const planPath = path.join(fDir, "plan.md");
				if (fs.existsSync(planPath)) {
					const plan = fs.readFileSync(planPath, "utf8");
					const hasArchitectureTags =
						plan.includes("<architecture>") && plan.includes("</architecture>");
					assertCondition(
						`Feature [${feature}] uses Aizen-Tags (XML) in plan.md`,
						hasArchitectureTags,
					);
				} else {
				...
				if (fs.existsSync(tasksDir)) {
					...
					if (wps.length > 0) {
						...
						assertCondition(
							`Feature [${feature}] WPs use Aizen-Tags (XML) task prompts`,
							hasTaskTags,
						);
					}
				}
		...
		assertCondition(
			"Isolated worktrees directory initialized (.worktrees/)",
			fs.existsSync(path.join(projectRoot, ".worktrees")),
			"medium",
		);
		assertCondition(
			"Aizen-Pulse Server config exists",
			fs.existsSync(path.join(saDir, "dashboard", "server.js")),
			"low",
		);
		...
		const scoreColor =
			percentage >= 90 ? chalk.green.bold : percentage >= 70 ? chalk.yellow.bold : chalk.red.bold;

		console.log(
			`Shield Compliance: ${scoreColor(`${percentage}%`)} (${passedChecks}/${totalChecks} constraints met)\n`,
		);

		if (percentage < 80) {
			console.log(
				chalk.red(
					"[CRITICAL] Quality Gate Violation. Agent behavior is deviating from Aizen-Gate Shield protocols.",
				),
			);
			console.log(
				`Recommendation: Re-run ${chalk.bold("npx aizen-gate install")} or run ${chalk.bold("npx aizen-gate doctor")} to repair.`,
			);
		} else if (percentage < 100) {
			console.log(
				chalk.yellow("[NOTICE] Protocol Coherence Degradation. Minor protocol gaps detected."),
			);
		} else {
			console.log(
				chalk.green(
					"[EXCELLENT] Agents are performing at Aizen-Gate ELITE level. Zero protocol leakage. ⛩️",
				),
			);
		}
		...
		console.error(chalk.red(`\n[Shield] Benchmark failed to run: ${(err as Error).message}`));
