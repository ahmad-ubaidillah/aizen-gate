...
export class WorktreeManager {
	...
	createWorktree(featureSlug: string, wpId: string, baseWp: string | null = null): string {
		if (!this.isGitRepo()) {
		...
		try {
			...
			if (fs.existsSync(sharedConst)) {
				try {
					...
					console.log(`[WorktreeManager] ⚠ Could not link Constitution: ${(_e as Error).message}`);
				}
			}

			return worktreePath;
		} catch (e) {
			console.error(`[WorktreeManager] Failed to create worktree: ${(e as Error).message}`);
			throw e;
		}
	}
	...
	removeWorktree(featureSlug: string, wpId: string): void {
		const branchName = `${featureSlug}-${wpId}`;
		const worktreePath = path.join(this.worktreesDir, branchName);
		...
		try {
			...
			console.error(`[WorktreeManager] Failed to remove worktree: ${(e as Error).message}`);
		}
	}

	autoCommitWP(branchName: string, wpId: string, message?: string): void {
		const worktreePath = path.join(this.worktreesDir, branchName);
		...
		try {
			...
			if (!(e as Error).message.includes("nothing to commit")) {
				console.error(`[WorktreeManager] Commit failed for ${wpId}: ${(e as Error).message}`);
			}
		}
	}
}
