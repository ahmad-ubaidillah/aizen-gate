 ...
 */
export async function runDiscoveryWizard(projectRoot: string): Promise<void> {
	...
	try {
		...
		try {
			...
			if (errorMessage?.includes("already exists")) {
				...
				console.log(chalk.yellow(`  ℹ Could not create branch: ${errorMessage}`));
			}
		}
		...
		cancel(`Failed to initialize discovery: ${(err as Error).message}`);
	}
}
 ...
 */
async function getNextFeatureId(projectRoot: string): Promise<string> {
...
function generateInitialSpec(name: string, objective: string): string {
	return `# Spec: ${name}
