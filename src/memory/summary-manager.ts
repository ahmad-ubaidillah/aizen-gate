...
export class SummaryManager {
  ...
  constructor(projectRoot: string, config: SummaryManagerConfig = {}) {
    ...
    console.log(chalk.cyan(`[SummaryManager] Initialized with threshold: ${this.messageThreshold} messages`));
  }
  ...
  generateSummary(messages: Message[]): string {
    ...
    for (const msg of messages) {
      ...
      if (decisionKeywords.some(kw => content.includes(kw))) {
        ...
        const decisionMatch = msg.content.match(/(?:decided|chosen|selected|agreed|will use|going with)[:\s]+(.+?)(?:\.|$)/i);
      ...
      if (progressKeywords.some(kw => content.includes(kw))) {
        const progressMatch = msg.content.match(/(?:completed|finished|done|started|implemented|created)[:\s]+(.+?)(?:\.|$)/i);
      ...
      if (questionKeywords.some(kw => content.includes(kw))) {
        const questionMatch = msg.content.match(/(?:should we|need to decide|how do we|what about)[:\s]+(.+?)(?:\?|$)/i);
  ...
  getSummary(sessionId: string): string {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to get summary: ${(error as Error).message}`));
      return '';
    }
  }
  ...
  getAllSummaries(sessionId: string): SummaryRecord[] {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to get all summaries: ${(error as Error).message}`));
      return [];
    }
  }
  ...
  trackTokenUsage(sessionId: string, agent: string, tokens: number): void {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to track token usage: ${(error as Error).message}`));
      throw error;
    }
  }
  ...
  getTokenUsage(sessionId: string): TokenUsage[] {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to get token usage: ${(error as Error).message}`));
      return [];
    }
  }
  ...
  getTotalTokenUsage(sessionId: string): number {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to get total token usage: ${(error as Error).message}`));
      return 0;
    }
  }
  ...
  getTokenUsageByAgent(sessionId: string): Record<string, number> {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to get token usage by agent: ${(error as Error).message}`));
      return {};
    }
  }
  ...
  processMessages(sessionId: string, messages: Message[]): string | null {
    if (this.shouldGenerateSummary(messages.length)) {
      ...
      this.storeSummary(sessionId, summary, messages.length - this.messageThreshold + 1, messages.length);
      return summary;
    }
    return null;
  }
  ...
  clearSession(sessionId: string): void {
    try {
      ...
      console.error(chalk.red(`[SummaryManager] Failed to clear session: ${(error as Error).message}`));
      throw error;
    }
  }
 ...
 */
export function createSummaryManager(projectRoot: string, config?: SummaryManagerConfig): SummaryManager {
