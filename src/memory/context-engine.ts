...
export class ContextEngine {
	...
	private MAX_FILE_LINES: number;
	private budget: any;
	private filter: any;
	private memory: any;

	constructor(projectRoot: string, limits: any = {}) {
		...
		this.MAX_FILE_LINES = limits.maxFileLines || Number(process.env.AZ_MAX_FILE_LINES) || 150;
	 ...
	 */
	processToolOutput(commandType: string, rawOutput: string) {
	 ...
	 */
	async sharpenPrompt(text: string) {
	 ...
	 */
	async getSubagentContext(taskId: string | null = null) {
...
`;
	 ...
	 */
	async assembleWPContext(featureDir: string, _wp: any) {
...
export const ContextManager = ContextEngine;
