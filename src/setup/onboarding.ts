...
import { runEnhancedOnboarding } from "./onboarding/index.js";
import { runComprehensiveOnboarding } from "./onboarding/steps/comprehensive-setup.js";

/**
 * Main onboarding entry point
 * Can run different types of onboarding based on options
 */
export async function runOnboarding(
	projectRootOrOptions?: string | { comprehensive?: boolean; projectRoot?: string },
): Promise<void> {
	// Handle both old signature (string) and new signature (object)
	let options: { comprehensive?: boolean; projectRoot?: string } = {};
	let _projectRoot = process.cwd();

	if (typeof projectRootOrOptions === "string") {
		// Old signature: runOnboarding(projectRoot)
		_projectRoot = projectRootOrOptions;
		options = {};
	} else if (projectRootOrOptions) {
		// New signature: runOnboarding({ comprehensive: true, projectRoot: ... })
		options = projectRootOrOptions;
		_projectRoot = options.projectRoot || process.cwd();
	}

	// If comprehensive flag is set, run the new comprehensive onboarding
	if (options.comprehensive) {
		await runComprehensiveOnboarding();
		return;
	}

	// Otherwise, run the original wizard
