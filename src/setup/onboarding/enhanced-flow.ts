...
export async function runEnhancedInstallFlow(
  ...
  if (!options.skipPrerequisites) {
    ...
    try {
      ...
      displayAizenError(
        AizenError.from(error as Error, AizenErrorCode.PROCESS_FAILED),
      );
    ...
    if (!prereqResults.criticalPassed) {
      ...
      const continueAnyway = await confirm({
        message:
          "Continue anyway? (Not recommended - some features may not work)",
        initialValue: false,
      });
  ...
  if (await fs.pathExists(existingConfigPath)) {
    try {
      ...
      console.log(
        chalk.dim(
          `  Last configured: ${existingConfig.timestamp || "Unknown"}`,
        ),
      );
      console.log();
      ...
      if (useExisting) {
        ...
        return await quickSetup(
          projectRoot,
          existingConfig as OnboardingConfig,
        );
      }
    } catch {
      displayWarning(
        "Existing configuration is invalid. Creating fresh setup...",
      );
      console.log();
    }
  }
  ...
  if (!scaffoldSuccess) {
    ...
    if (rollback && !isCancel(rollback)) {
      try {
        ...
        displayWarning(
          "Could not fully rollback. Manual cleanup may be needed.",
        );
      }
    }
  ...
  if (runTutorial && !isCancel(runTutorial)) {
    ...
    console.log();
    displayInfo(
      "Tutorial skipped. Run 'npx aizen-gate tutorial' anytime to learn more.",
    );
    console.log();
  }
...
async function runInteractiveSetup(
  ...
  const selectedChatLang = LANGUAGE_OPTIONS.find(
    (l) => l.value === chatLanguage,
  );
  ...
  const resolvedOutputLang =
    outputLanguage === "same" ? chatLanguage : outputLanguage;
  displaySuccess(
  ...
  const selectedStyle = DEV_TYPE_OPTIONS.find(
    (opt) => opt.value === projectStyle,
  );
...
async function runScaffolding(
  ...
  try {
    ...
    if (!tracker) {
      localTracker.finish();
      displayAizenError(
        AizenError.from(error as Error, AizenErrorCode.INSTALL_DIRECTORY),
      );
    }
    return false;
  }
  ...
  try {
    ...
    if (!tracker) {
      localTracker.finish();
      displayAizenError(
        AizenError.from(error as Error, AizenErrorCode.INSTALL_CONFIG),
      );
    }
    return false;
  }
...
function displayCompletion(config: OnboardingConfig): void {
  ...
  console.log(
    chalk.cyan.bold(
      "  ⛩️  Welcome to Aizen-Gate! Let's build something amazing.",
    ),
  );
  console.log();
}
...
export async function runEnhancedStartFlow(
  ...
  if (await fs.pathExists(configPath)) {
    try {
      ...
      displayInfo(`Project: ${config.projectName || "Aizen-Gate"}`);
      console.log(chalk.dim(`  Version: ${config.version || "Unknown"}`));
      console.log();
    } catch (error) {
      displayWarning(
        "Configuration file is corrupted. Run 'npx aizen-gate install' to fix.",
      );
      console.log();
  ...
  if (!config) {
    ...
    console.log();
    console.log(
      chalk.white("  It looks like Aizen-Gate hasn't been set up yet."),
    );
    console.log();
  ...
  try {
    ...
    console.log(chalk.dim(`  Open manually: ${aizenMdPath}`));
  }
  ...
  if (!prdExists) {
    ...
    console.log(
      chalk.white("  A Product Requirements Document (PRD) helps you:"),
    );
  ...
  console.log(
    chalk.dim(
      "  💡 Pro tip: Use 'npx aizen-gate --help' to see all available commands",
    ),
  );
  console.log();
