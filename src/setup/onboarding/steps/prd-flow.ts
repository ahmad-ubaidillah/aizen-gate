 ...
 */
const QUICK_START_PRD: Record<string, string> = {
...
`,
	api: `# API/Backend Service - Product Requirements Document

## 1. Project Overview
- **Project Name:** Backend API Service
- **Type:** REST/GraphQL API
- **Core Feature:** Provide data endpoints for client applications
- **Target Users:** Web/Mobile app developers

## 2. User Stories
| As a... | I want to... | So that... |
| :------ | :------------ | :---------- |
| Developer | Access data via API | I can build frontends |
| Developer | Create/Update/Delete records | I can manage data |
| Developer | Authenticate requests | Only authorized users access data |

## 3. Functional Requirements
| ID | Feature | Description |
| :-- | :------ | :---------- |
| F1 | REST Endpoints | CRUD operations for resources |
| F2 | Authentication | JWT or API key based auth |
| F3 | Data Validation | Validate incoming request data |
| F4 | Error Handling | Return proper error responses |
| F5 | Documentation | API docs (OpenAPI/Swagger) |
 ...
 */
async function _handleHasPRD(projectRoot: string): Promise<any> {
	note(
 ...
 */
async function _handleNoPRD(projectRoot: string): Promise<any> {
	...
	const wantsQuickStart = await select({
		...
		options: QUICK_START_OPTIONS as any,
	});
 ...
 */
async function handleManualPRD(
	_projectRoot: string,
	_prdFolderPath: string,
	brainstormingPath: string,
	prdFilePath: string,
	...
	const createMethod = await select({
		...
		options: PRD_CREATE_OPTIONS as any,
	});
 ...
 */
export async function validatePRDReady(projectRoot: string): Promise<any> {
