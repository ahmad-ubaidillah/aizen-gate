...
export class KanbanScaffolder {
  ...
  async scaffold(): Promise<void> {
    ...
    const sourceWorkflowsDir = path.join(
      __dirname,
      "..",
      "..",
      "infrastructure",
    ...
    const sourceRulesDir = path.join(
      __dirname,
      "..",
      "..",
      "infrastructure",
    ...
    for (const dir of kanbanDirs) {
      if (!fs.existsSync(dir)) {
        ...
        console.log(
          chalk.gray(`   ✔ Created .agent/${path.relative(agentDir, dir)}`),
        );
      }
    }
    ...
    const constitutionPath = path.join(
      agentDir,
      ".shared/PROJECT_CONSTITUTION.md",
    );
    if (!fs.existsSync(constitutionPath)) {
      ...
      console.log(
        chalk.gray(`   ✔ Created .agent/.shared/PROJECT_CONSTITUTION.md`),
      );
    }
  ...
  private async copyDirectory(
    ...
    const resolvedSrc = fs.existsSync(src)
      ? src
    ...
    if (!fs.existsSync(resolvedSrc)) {
      console.log(
        chalk.yellow(
          `   ⚠️ Source not found: ${resolvedSrc} (skipping ${label})`,
        ),
      );
      return;
    }
    ...
    if (copiedCount > 0) {
      console.log(
        chalk.gray(`   ✔ Copied ${copiedCount} ${label} to .agent/${label}/`),
      );
    } else {
