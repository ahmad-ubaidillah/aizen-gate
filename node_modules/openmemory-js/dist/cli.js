...
async function get_existing_tables() {
    const tables = await (0, db_1.all_async)(`SELECT name FROM sqlite_master WHERE type='table'`);
    return new Set(tables.map((t) => t.name));
}
async function get_existing_indexes() {
    const indexes = await (0, db_1.all_async)(`SELECT name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'`);
...
async function run_migrations() {
    ...
    for (const [table_name, schema] of Object.entries(SCHEMA_DEFINITIONS)) {
        if (!existing_tables.has(table_name)) {
            ...
            for (const stmt of statements) {
                if (stmt.trim()) {
                    await (0, db_1.run_async)(stmt.trim());
                }
            }
            created_tables++;
        }
    ...
    for (const index_sql of INDEX_DEFINITIONS) {
        ...
        if (index_name && !existing_indexes.has(index_name)) {
            ...
            await (0, db_1.run_async)(index_sql);
            created_indexes++;
        }
    }
