        ...
        await Promise.all(parts.map(async (part) => {
            for (const m of part) {
                ...
                if (f < 0.7) {
                    ...
                    const vec_row = await db_1.vector_store.getVector(m.id, sector);
                    if (vec_row && vec_row.vector) {
                        ...
                        if (before_len > 0) {
                            ...
                            if (new_vec.length < before_len) {
                                await db_1.vector_store.storeVector(m.id, sector, new_vec, new_vec.length);
                                compressed = true;
                                tot_comp++;
                            }
                ...
                if (f < Math.max(0.3, cfg.cold_threshold)) {
                    ...
                    await db_1.vector_store.storeVector(m.id, sector, fp.vector, fp.vector.length);
...
const on_query_hit = async (mem_id, sector, reembed) => {
    ...
    if (cfg.regeneration_enabled && reembed) {
        const vec_row = await db_1.vector_store.getVector(mem_id, sector);
        if (vec_row && vec_row.vector) {
            const vec = typeof vec_row.vector === "string"
                ? JSON.parse(vec_row.vector)
                : vec_row.vector;
            if (Array.isArray(vec) && vec.length <= 64) {
                try {
                    ...
                    await db_1.vector_store.storeVector(mem_id, sector, new_vec, new_vec.length);
                    updated = true;
                }
                catch (e) { }
            }
