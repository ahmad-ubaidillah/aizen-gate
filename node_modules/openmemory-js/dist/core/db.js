...
if (is_pg) {
    ...
    const init = async () => {
        ...
        await pg.query(`create table if not exists ${v}(id uuid,sector text,user_id text,v vector,dim integer not null,primary key(id,sector))`);
        await pg.query(`create index if not exists openmemory_vectors_hnsw_idx on ${v} using hnsw (v vector_cosine_ops)`);
        ...
        await pg.query(`create table if not exists "${sc}"."temporal_facts"(id uuid primary key,user_id text,subject text not null,predicate text not null,object text not null,valid_from bigint not null,valid_to bigint,confidence double precision not null check(confidence >= 0 and confidence <= 1),last_updated bigint not null,metadata text,unique(subject,predicate,object,valid_from))`);
        await pg.query(`create index if not exists temporal_facts_user_idx on "${sc}"."temporal_facts"(user_id)`);
        await pg.query(`create table if not exists "${sc}"."temporal_edges"(id uuid primary key,source_id uuid not null,target_id uuid not null,relation_type text not null,valid_from bigint not null,valid_to bigint,weight double precision not null,metadata text,foreign key(source_id) references "${sc}"."temporal_facts"(id),foreign key(target_id) references "${sc}"."temporal_facts"(id))`);
        await pg.query(`create index if not exists temporal_facts_subject_idx on "${sc}"."temporal_facts"(subject)`);
        await pg.query(`create index if not exists temporal_facts_predicate_idx on "${sc}"."temporal_facts"(predicate)`);
        await pg.query(`create index if not exists temporal_facts_validity_idx on "${sc}"."temporal_facts"(valid_from,valid_to)`);
        await pg.query(`create index if not exists temporal_facts_composite_idx on "${sc}"."temporal_facts"(subject,predicate,valid_from,valid_to)`);
        await pg.query(`create index if not exists temporal_edges_source_idx on "${sc}"."temporal_edges"(source_id)`);
        await pg.query(`create index if not exists temporal_edges_target_idx on "${sc}"."temporal_edges"(target_id)`);
        await pg.query(`create index if not exists temporal_edges_validity_idx on "${sc}"."temporal_edges"(valid_from,valid_to)`);
        ...
        if (cfg_1.env.vector_backend === "valkey") {
            ...
            exports.vector_store = vector_store = new postgres_1.PostgresVectorStore({ run_async, get_async, all_async }, v.replace(/"/g, ""), true);
    ...
    db.serialize(() => {
        ...
        db.run(`create table if not exists temporal_facts(id text primary key,user_id text,subject text not null,predicate text not null,object text not null,valid_from integer not null,valid_to integer,confidence real not null check(confidence >= 0 and confidence <= 1),last_updated integer not null,metadata text,unique(subject,predicate,object,valid_from))`);
        db.run("create index if not exists idx_temporal_user on temporal_facts(user_id)");
    ...
    exports.q = q = {
        ...
        upd_mean_vec: {
            run: (...p) => exec("update memories set mean_dim=?,mean_vec=? where id=?", [
                p[1],
                p[2],
                p[0],
            ]),
