# Aizen-Gate Playbook: az-specify
...
3. **Branch Initialization**: Create a dedicated git branch for the feature: `git checkout -b feature/{feature-slug}`. This ensures all subsequent research, planning, and implementation are isolated.
4. **Generate spec.md**: Create `aizen-gate/specs/{feature-slug}/spec.md` with:
...
- **Research Needed**: Flag areas requiring `az-research`.

## Exit Criteria

- A fully populated `spec.md` exists in a new feature directory.
- The Scrum Master returns control to the user to either:
  a) Run `az-research` if there are architectural unknowns.
  b) Run `az-plan` if ready to commit to a tech stack.
