...
function install({ home, force = false, projectPath } = {}) {
  const { agentsDir, commandsDir, dataDir, kilnthreeDir, skillsDir, templatesDir } = resolvePaths(home);

  fs.mkdirSync(agentsDir, { recursive: true });
  fs.mkdirSync(commandsDir, { recursive: true });
  fs.mkdirSync(dataDir, { recursive: true });
  fs.mkdirSync(kilnthreeDir, { recursive: true });
  ...
  for (const { srcDir, destDir, ext } of copyJobs) {
    ...
    for (const filename of filenames) {
      ...
      if (destChecksum === srcChecksum) {
        ...
        console.error(`[kilnthree] skipping ${destPath} (user-edited; use --force to overwrite)`);
    ...
    for (const file of agentFiles) {
      if (file.startsWith('kw-') && file.endsWith('.md')) {
        ...
        try {
          ...
          console.error(`[kilnthree] removing legacy agent: ${file}`);
        } catch {
  ...
  // Copy names.json to kilnthreeDir
  const namesSrc = path.join(ASSETS_DIR, 'names.json');
  const namesDest = path.join(kilnthreeDir, 'names.json');
  if (force || !fs.existsSync(namesDest)) {
    ...
    if (destChecksum === srcChecksum) {
      ...
      console.error(`[kilnthree] skipping ${namesDest} (user-edited; use --force to overwrite)`);
  ...
  writeManifest({
    ...
    files,
    protocolMarkers: {
      begin: 'kiln3:protocol:begin',
      end: 'kiln3:protocol:end',
    },
