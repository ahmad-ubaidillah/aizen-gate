...
const BEGIN_RE = /<!-- kiln3:protocol:begin v([\d.]+) -->/;
const END_RE = /<!-- kiln3:protocol:end -->/;

function buildBlock(content, version) {
  const inner = content.endsWith('\n') ? content : `${content}\n`;
  return `<!-- kiln3:protocol:begin v${version} -->\n${inner}<!-- kiln3:protocol:end -->\n`;
}
...
function replaceProtocol(filePath, content, version) {
  ...
  if (!block) {
    throw new Error(`kilnthree: no protocol block found in ${filePath}`);
  }
...
/**
 * Removes the Kiln v3 protocol block from a CLAUDE.md file.
