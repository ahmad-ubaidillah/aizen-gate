# Aizen-Gate Playbook: za-implement
...
2. **Worktree Initialization**: Ensure you are in the isolated git worktree created for this WP (e.g., `.worktrees/001-feature-WP01`). If not, the `[SA]` should have created one via `za-auto` or `WorktreeManager`.
...
6. **Lane Transition**: Move the WP from `doing` to `review` when complete by updating the frontmatter in the WP file or using the dashboard API.
