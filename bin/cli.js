...
program
	...
	.option("--fix", "Attempt to auto-repair detected issues")
	.action(async (options) => {
...
program
	...
	.action(async () => {
		const { SkillWatcher } = require("../scripts/skill-watcher");
...
program
	...
	.action(async () => {
		...
		console.log(
			chalk.gray(`Invoke agent: "Read aizen-gate/commands/az-specify.md and start discovery."`),
		);
	});

program
	.command("discuss")
	.description("Pre-planning dialogue to capture nuanced preferences and resolve ambiguities")
	.action(async () => {
		console.log(chalk.red("\n--- ⛩️ [Aizen] Launching az-discuss Playbook ---\n"));
		console.log(
			chalk.gray(
				`Invoke agent: "Read aizen-gate/commands/az-discuss.md and start the discussion phase."`,
			),
		);
	});
...
program
	...
	.action(async () => {
		...
		console.log(
			chalk.gray(`Invoke agent: "Read aizen-gate/commands/az-research.md and begin research."`),
		);
	});
...
program
	...
	.action(async () => {
		...
		console.log(
			chalk.gray(
				`Invoke agent: "Read aizen-gate/commands/az-plan.md and generate an architectural plan."`,
			),
		);
	});
...
program
	.command("tasks")
	.description("Decompose plan into Work Packages (WPs)")
	.option("-f, --feature <slug>", "Feature to decompose")
	.action(async (options) => {
		const { analyzeAndDecompose } = require("../scripts/spec-decomposer");
		const shouldDecompose = await analyzeAndDecompose(process.cwd(), options.feature);

		if (shouldDecompose) {
			console.log(
				chalk.red(
					"\n[Aizen] Stop! Please decompose this feature into sub-specs before proceeding to tasks.\n",
				),
			);
			return;
		}
		...
		console.log(
			chalk.gray(`Invoke agent: "Read aizen-gate/commands/az-tasks.md and generate WPs."`),
		);
	});
...
program
	...
	.action(async (wpId) => {
		...
		console.log(
			chalk.gray(`Invoke agent: "Read aizen-gate/commands/az-review.md and perform review."`),
		);
	});
...
program
	...
	.action(async () => {
		...
		console.log(
			chalk.gray(
				`Invoke agent: "Read aizen-gate/commands/az-merge.md and start consensus sequence."`,
			),
		);
	});
...
program
	...
	.action(async () => {
		...
		if (typeof report === "string") {
			...
			console.log(
				`${chalk.bold("Input Tokens:")} ${report.input_tokens.toLocaleString()} (${((report.input_tokens / report.total_tokens) * 100).toFixed(1)}%)`,
			);
			console.log(
...
program
	...
	.action(async (action, key, value) => {
		...
		if (!fs.existsSync(configPath))
			return console.log(chalk.red("❌ Config file not found. Run install first."));

		const config = yaml.load(fs.readFileSync(configPath, "utf8"));

		if (action === "list") {
			console.log(chalk.red.bold("\n--- ⛩️ [AZ] Project Configuration ---\n"));
			Object.entries(config).forEach(([k, v]) => {
				console.log(`${chalk.bold(k.padEnd(20))}: ${v}`);
			});
		} else if (action === "get") {
			if (!key) return console.log(chalk.red("❌ Key required."));
			console.log(`${chalk.bold(key)}: ${config[key]}`);
		} else if (action === "set") {
			if (!key || !value) return console.log(chalk.red("❌ Key and value required."));
			// Handle nested keys like model.family -> model_family
...
program
	.command("checklist")
	.description("Generate structured QA checklist from Spec ACs")
	.option("-f, --feature <slug>", "Specify the feature slug")
	.action(async (options) => {
		const { generateChecklist } = require("../scripts/checklist-gen");
		await generateChecklist(process.cwd(), options.feature);
	});

program
	.command("todos")
	.description("Scan codebase for // TODO: and // FIXME: comments")
	.action(async () => {
		const { scanTodos } = require("../scripts/todo-scanner");
...
skillCmd
	...
	.action(async (links) => {
		...
		console.log(chalk.red.bold("\n--- ⛩️ [AZ] Skill Creation Phase ---\n"));
		await runSkillCreator(process.cwd(), linkArray);
	});
...
program.arguments("<unrecognized>").action(async (cmd) => {
	...
	const plainPath = path.join(process.cwd(), "aizen-gate", "commands", `${cmd}.md`);

	const playbookPath = fs.existsSync(azPath) ? azPath : fs.existsSync(plainPath) ? plainPath : null;
