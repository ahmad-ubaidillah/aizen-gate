...
describe('TaskCLI & TaskSearch - Suite', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(testRoot, 'backlog', 'config.yml'), 'statuses: [Todo, In Progress, Done]\ndefinition_of_done:\n  - AC met\n  - Tests pass\n');
  ...
  it('Task 7: Fuzzy Search', async () => {
    await cli.create('Build Login API', { description: 'Use JWT for authentication', priority: 'high' });
    await cli.create('Design Login UI', { description: 'Use React components', priority: 'medium', assignee: '@ui-agent' });
