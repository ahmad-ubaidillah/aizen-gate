...
async function runVerifier(projectRoot, featureName) {
    ...
    if (doneWps.length === 0) {
        console.log(chalk.yellow(`[Aizen] No 'done' or 'review' Work Packages found in feature: ${activeFeature}.`));
        return;
    }
    ...
    for (const wp of doneWps) {
        ...
        const acItems = wp.content.match(/- \[ \].*|- \[x\].*/g) || ["No specific AC found in WP content."];
        ...
        if (decision === 'fail') {
            const issueDetail = await input({ message: 'Describe the discrepancy (Expected vs Actual):' });
            ...
            console.log(chalk.white(`  <instructions>Read WP ${wp.filePath} and implementation artifacts. Generate a fix plan.</instructions>`));
