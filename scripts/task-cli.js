...
`;
  ...
  async list() {
    ...
    files.forEach(f => {
      ...
      if (match) {
        ...
        const statusColor = fm.status === 'Done' ? chalk.green : (fm.status === 'In Progress' ? chalk.yellow : chalk.white);
        console.log(`${chalk.bold(fm.id)} [${statusColor(fm.status)}] | Assignee: ${fm.assignee} | ${f.replace(/aizen-\d+ - (.*)\.md/, '$1')}`);
      }
    });
  }
  ...
  async edit(id, options) {
    ...
    if (match) {
      ...
      if (options.status) { fm.status = options.status; changed = true; }
      if (options.priority) { fm.priority = options.priority; changed = true; }
      if (options.assignee) { fm.assignee = options.assignee; changed = true; }
