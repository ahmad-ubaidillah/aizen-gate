...
async function runQuickFlow(projectRoot, description) {
    ...
    if (!fs.existsSync(historyPath)) {
        await fs.writeFile(historyPath, `# ⚡ Quick Flow History\n\nLog of all rapid implementations.\n\n`);
    }

    try {
        // 1. Structural Prompt for the Agent (In case the agent is calling this command)
        console.log(chalk.white(`\n<quick_flow_task>`));
        console.log(chalk.white(`  <instruction>Implement the following task directly: ${description}</instruction>`));
        console.log(chalk.white(`  <workflow>1. Analysis -> 2. Implementation -> 3. Local Verification -> 4. Commit</workflow>`));
        console.log(chalk.white(`  <constraints>Stay within the minimal scope of the task description.</constraints>`));
        ...
        console.log(chalk.yellow(`[Aizen] Once finished, use "git commit -m '${description}'" or run "npx aizen-gate quick --commit".\n`));
