...
class WorktreeManager {
  ...
  createWorktree(featureSlug, wpId, baseWp = null) {
    ...
    try {
      if (baseWp) {
        ...
        execSync(`git worktree add -b ${branchName} ${worktreePath} ${baseBranch}`, { stdio: 'pipe', cwd: this.projectDir });
      } else {
  ...
  listWorktrees() {
    ...
    try {
      const output = execSync('git worktree list --porcelain', { cwd: this.projectDir, encoding: 'utf-8' });
  ...
  removeWorktree(featureSlug, wpId) {
    ...
    try {
      ...
      execSync(`git worktree remove --force ${worktreePath}`, { stdio: 'pipe', cwd: this.projectDir });
      console.log(`[WorktreeManager] Cleaned up worktree for ${wpId}`);
      // Note: We leave the branch intact so it can be merged via 'za-merge' later.
    } catch (e) {
