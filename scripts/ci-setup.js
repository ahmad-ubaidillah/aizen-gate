...
async function setupCI(projectRoot) {
    ...
    const ciFilePath = path.join(githubDir, 'za-compliance.yml');
...
`;
    ...
    try {
        ...
        if (fs.existsSync(ciFilePath)) {
            console.log(chalk.yellow(`[Notice] ${ciFilePath} already exists. Skipping overwrite to prevent unmerging custom CI logic.`));
        } else {
