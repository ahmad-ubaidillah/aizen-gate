...
class ContextManager {
    ...
    async auditContext() {
        ...
        for (const file of files) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                if (status === 'ROT') {
                    console.log(chalk.yellow(`[SA] Warning: ${file} is becoming bloated (${lineCount} lines). Compression recommended.`));
                }
            }
        }
        return reports;
    ...
    async snapshotState(tag = 'session') {
        ...
        const snapshotDir = path.join(this.projectRoot, 'aizen-gate/data/snapshots', `${tag}-${timestamp}`);
