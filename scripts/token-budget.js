...
class TokenBudget {
  ...
  async enforce(phase, content) {
    ...
    console.log(chalk.yellow(`[Aizen] Budget overrun in "${phase}" (${currentTokens} > ${limit}). Enforcing limit...`));
  ...
  async getReport() {
    ...
    const summary = ledger.reduce((acc, entry) => {
      ...
      acc.phases[entry.phase] = (acc.phases[entry.phase] || 0) + (entry.tokensIn + entry.tokensOut);
      return acc;
