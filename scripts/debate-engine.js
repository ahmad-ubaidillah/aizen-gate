...
class DebateEngine {
  constructor(projectRoot, featureDir) {
    ...
    this.historyPath = path.join(projectRoot, 'aizen-gate', 'shared', 'debates', `${this.debateId}.md`);
    this.proposals = [];
    this.critiques = [];
    this.moderations = [];
    this.synthesis = null;
  ...
  async addProposal(content) {
    this.proposals.push({ role: 'Architect (Thesis)', content, timestamp: new Date().toISOString() });
    await this.persist();
  }
  ...
  async addCritique(content) {
    this.critiques.push({ role: 'Developer (Antithesis)', content, timestamp: new Date().toISOString() });
    await this.persist();
  }
  ...
  async addModeration(content) {
    this.moderations.push({ role: 'Socrates (Moderator)', content, timestamp: new Date().toISOString() });
    await this.persist();
  }
