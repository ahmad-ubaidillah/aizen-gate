...
async function ingestDocument(projectRoot, targetPath) {
    ...
    if (ext === '.md' || ext === '.txt') {
        ...
        console.log(chalk.green(`✔ Ingested ${ingested} memory chunks from ${path.basename(targetPath)}`));
    } else {
        console.log(chalk.red(`[Aizen] Unsupported chunk format: ${ext}. Currently supports .md and .txt.`));
    }
}
